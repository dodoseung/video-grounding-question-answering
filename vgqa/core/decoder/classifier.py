...
from ..language.bert_module import BertLMPredictionHead, BertLayer_Cross
...
class TemporalSampling(nn.Module):
    ...
    def __init__(self, width: int):
        ...
        self.layer_ca = nn.ModuleList(
            [BertLayer_Cross(self.bert_config) for _ in range(self.bert_config.num_layers)]
        )
...
class SpatialActivation(nn.Module):
    ...
    def forward(self, input: torch.Tensor, init_q: Optional[torch.Tensor] = None) -> Tuple[torch.Tensor, torch.Tensor]:
        input = input.permute(0, 2, 3, 1)
        x = input.reshape(input.size(0), -1, 256)
        query = (
            torch.zeros(x.size(0), 1, x.size(-1), device=x.device)
            if init_q is None
            else init_q.repeat(x.size(0), 1, 1)
        )

        for i in range(self.bert_config.num_layers):
            query, att_map = self.layer_ca[i](query, x)
        att_map = att_map.sum(1).squeeze(1).sigmoid()
        att_min = att_map.min(dim=1, keepdim=True)[0]
        att_max = att_map.max(dim=1, keepdim=True)[0]
        att_map = (att_map - att_min) / (att_max - att_min + 1e-6)
