...
from .vidstg_data_utils import SENTENCE_SPLIT_REGEX, make_vidstg_input_clip
from .words import replace_dict


def merge_anno(rootdir: str):
...
def clean_anno(data: List[Dict[str, Any]]):
...
class VidSTGDataset(data.Dataset):
    ...
    def __init__(self, cfg, split, transforms=None) -> None:
        ...
        self.all_gt_data = clean_anno(all_gt_data)
        self.vocab = None
    ...
    def get_video_info(self, index: int):
        ...
        info['height'] = data_item['height']
        info['width'] = data_item['width']
        return info

    def load_frames(self, data_item: Dict[str, Any], load_video: bool = True):
        video_id = data_item['vid']
        ...
        if load_video:
            ...
            frames_np = frames_np[frame_ids]
            rate = frames_np.shape[2] / frames_np.shape[1]
            frames = [
                Resize((resolution, min(int(resolution * rate), int(resolution * max_rate))), antialias=True)(
                    ToTensor()(frame)
                )
            ...
            frames = []
            frame_paths = [
                os.path.join(self.frame_dir, video_id, 'img_{:05d}.jpg'.format(fid)) for fid in frame_ids
            ]
    ...
    def __getitem__(self, index: int):
        """In training, sample a random clip; in testing, chunk to a set of clips."""
        ...
        if len(action_idx) == 0:
            return self.__getitem__((index + 1) % len(self.all_gt_data))

        start_idx, end_idx = action_idx[0], action_idx[-1]
        bbox_idx = [frame_ids[idx] - temp_gt[0] for idx in range(start_idx, end_idx + 1)]
        boxes_tensor = torch.from_numpy(data_item['bboxs'][bbox_idx]).reshape(-1, 4)
        assert boxes_tensor.shape[0] == len(action_idx)

        w, h = data_item['width'], data_item['height']
        boxes = BoxList(boxes_tensor, (w, h), 'xyxy')

        sentence = data_item['description'].lower()
        input_dict = {'frames': frames, 'boxs': boxes, 'text': sentence, 'actioness': data_item['actioness']}
    ...
    def load_data(self):
        ...
        for anno_id in tqdm(vstg_anno):
            ...
            if (
                len(gt_file['target_bboxs'])
                != gt_file['temp_gt']['end_fid'] - gt_file['temp_gt']['begin_fid'] + 1
            ):
                continue

            if (
                gt_file['ori_temp_gt']['end_fid'] < gt_file['used_segment']['begin_fid']
                or gt_file['ori_temp_gt']['begin_fid'] > gt_file['used_segment']['end_fid']
            ):
                continue

            video_id = gt_file['vid']
            ...
            frame_ids = [fid for fid in range(start_fid, end_fid + 1)]
    ...
    def make_data_pairs(self, anno_file):
        ...
        for anno in tqdm(sent_annos):
            ...
            data_pairs['frame_count'] = (
                anno['used_segment']['end_fid'] - anno['used_segment']['begin_fid'] + 1
            )

            data_pairs['temp_gt'] = deepcopy(anno['temporal_gt'])
            data_pairs['temp_gt']['begin_fid'] = (
                anno['temporal_gt']['begin_fid'] - anno['used_segment']['begin_fid']
            )
            data_pairs['temp_gt']['end_fid'] = (
                anno['temporal_gt']['end_fid'] - anno['used_segment']['begin_fid']
            )
            data_pairs['temp_gt']['end_fid'] = min(
                data_pairs['frame_count'] - 1, data_pairs['temp_gt']['end_fid']
            )
