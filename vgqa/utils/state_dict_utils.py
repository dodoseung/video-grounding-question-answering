...
def align_and_update_state_dicts(model_state_dict: Dict[str, torch.Tensor],
                                 loaded_state_dict: Dict[str, torch.Tensor],
                                 load_mapping: Dict[str, str],
                                 logger: logging.Logger) -> None:
    """Align and update a model state dict with loaded weights using suffix matching."""
    ...
    idxs[max_match_size == 0] = -1  # no match
    ...
    for idx_new, idx_old in enumerate(idxs.tolist()):
        if idx_old == -1:
            ...
            logger.info("NO-MATCHING of current module: {} of shape {}".format(key, tuple(model_state_dict[key].shape)))
            continue
        key = current_keys[idx_new]
...
def load_state_dict(model: torch.nn.Module,
                    ...
                    load_mapping: Dict[str, str],
                    logger: logging.Logger) -> None:
    """Load state dict into model with alignment and mapping (strict)."""
